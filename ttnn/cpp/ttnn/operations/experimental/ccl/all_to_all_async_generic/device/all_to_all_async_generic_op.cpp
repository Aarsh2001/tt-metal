...
void AllToAllAsyncGeneric::validate_with_output_tensors(
    ...
    TT_FATAL(
        input_tensor.padded_shape()[this->out_dim] % this->num_devices == 0,
    ...
    if (maybe_output_tensor.has_value()) {
        ...
        input_shape[this->in_dim] *= this->num_devices;
        input_shape[this->out_dim] /= this->num_devices;
        TT_FATAL(
    ...
    TT_FATAL(
        ...
        "{} dimension support only 16 padding, so must be dividable by 16. Input tensor shape {} , but has {} padding",
        in_dim,
        input_shape,
        input_shape[in_dim] % 32);
    TT_FATAL(
        out_dim != second_last_dim || output_shape[out_dim] % 16 == 0,
        "{} dimension support only 16 padding, so must be dividable by 16. Output tensor shape {} , but has {} padding",
        out_dim,
        output_shape,
        output_shape[out_dim] % 32);
    TT_FATAL(
        in_dim != last_dim || input_shape[in_dim] % 32 == 0,
        "{} dimension doesnt support padding, so must be dividable by 32. Input tensor shape {} , but has {} padding",
        in_dim,
        input_shape,
        input_shape[in_dim] % 32);
    TT_FATAL(
        out_dim != last_dim || output_shape[out_dim] % 32 == 0,
        "{} dimension doesnt support padding, so must be dividable by 32. Output tensor shape {} , but has {} padding",
        out_dim,
        output_shape,
        output_shape[out_dim] % 32);
}
