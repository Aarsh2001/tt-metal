...
void kernel_main() {
    ...
    const auto attention_sink_tile_shape = TensorTileShape(B, NQH, Sq_chunk_t, 1);
                ...
                for (uint32_t q_iter = 0; q_iter < q_chunks_per_core; ++q_iter) {
                    ...
                    */
                    ...
                    if constexpr (use_attention_sink) {
                        ...
                        // Attention sink has shape [B, NH, Sq_chunk, 1]
                        // Read the column vector for this batch and head
                        const uint32_t sink_tile_id = attention_sink_tile_shape.id_of(nb, nq, 0, 0);
                        for (uint32_t row = 0; row < Sq_chunk_t; ++row) {
                            noc_async_read_tile(sink_tile_id + row, attention_sink_reader, attention_sink_write_ptr);
