...
void ScaledDotProductAttention::validate(
    ...
    auto validate_attention_sink = [&]() {
        ...
        if (optional_input_tensors.size() > 2 && optional_input_tensors.at(2).has_value()) {
            ...
            // Attention sink should have shape [B, NH, Sq_chunk, 1] - a column vector for each Q chunk
            ...
            // The sequence dimension should match Q chunk size
            const auto q_chunk_size = this->get_q_chunk_size();
            TT_FATAL(
                sink_shape[2] == q_chunk_size,
                "Attention sink sequence dimension must match q_chunk_size. Got {}, expected {}",
                sink_shape[2],
                q_chunk_size);
            TT_FATAL(sink_shape[3] == TILE_WIDTH, "Attention sink last dimension must be a single tile width (32)");
        }
    };
