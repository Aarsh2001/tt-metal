...
bool TestConfigBuilder::should_skip_test(const ParsedTestConfig& test_config) const {
        ...
        for (const auto& p : p_config.patterns.value()) {
            if (p.iterations.has_value()) {
                ...
                if (p.type == "all_to_one") {
                    log_warning(
                        tt::LogTest,
                        "'iterations' specified alongside 'all_to_one' test, `iterations` will be followed instead of "
                        "auto-generating iterations based on number of devices");
                }
...
std::pair<std::vector<TrafficPatternConfig>, uint32_t> TestConfigBuilder::create_sync_patterns_for_topology(
    ...
    auto [multi_directional_hops, global_sync_val] = this->route_manager_.get_sync_hops_and_val(src_device, devices);
