...
def test_vae_qwenimage_decoder(
    *,
    ...
    torch_model = diffusers.models.autoencoders.AutoencoderKLQwenImage()
    torch_model.eval()

    in_channels = torch_model.config["z_dim"]

    ccl_manager = CCLManager(submesh_device, topology=ttnn.Topology.Linear)
    vae_parallel_config = VAEParallelConfig(tensor_parallel=ParallelFactor(factor=mesh_device.shape[1], mesh_axis=1))
    ...
    tt_input_tensor = tensor.from_torch(torch_input.permute(0, 2, 3, 1), device=submesh_device)

    with torch.no_grad():
        torch_output = torch_model.decode(torch_input.unsqueeze(2)).sample.squeeze(2)

    tt_out = tt_model(tt_input_tensor)

    tt_out_torch = tensor.to_torch(tt_out, mesh_axes=[None, None, None, None]).permute(0, 3, 1, 2)
    assert_quality(torch_output, tt_out_torch, pcc=0.9994, relative_rmse=0.036)
